...
actor TestImageDownloader: ImageDownloader {
    ...
    func fetchImage(with url: URL, forceRefresh: Bool, processingMethod: ImageProcessingMethod) async throws -> ImageDownloadResult {
        let task = Task<ImageDownloadResult, Error> {
            switch result {
            case .urlSessionError:
                let error = TestURLSessionError(message: TestImageDownloader.sessionErrorMessage)
                throw ImageFetchingError.responseError(
                    reason: .URLSessionError(error: error)
                )
            case .fail:
